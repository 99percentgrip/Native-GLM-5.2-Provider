...
- Standalone terminal composition accepts bracketed multiline paste without dropping a leading-newline prompt, presents it safely in the single-line composer, and keeps the complete composer border above the footer at supported terminal sizes.
...
- Package and ACP implementation version is `1.8.4` from `glm_acp.__version__`.
- GitHub release `v1.8.4` publishes the five supported frozen binaries, checksums, provenance attestations, Python distributions, Registry metadata, the icon, checksum-verifying Unix and Windows installers, and safe one-command uninstall support.
...
# expect: editable glm_acp metadata and glm_acp-1.8.4.dist-info
```
